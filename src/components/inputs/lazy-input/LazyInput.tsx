...
const LazyInput: ForwardRefRenderFunction<
  IAppLazyInputRef,
  IAppLazyInputProps
> = (props, ref) => {
  ...
  useImperativeHandle<IAppLazyInputRef, IAppLazyInputRef>(
    ref,
    () => ({
      setValue: (val: string) => {
  ...
  const checkEnterKey = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      checkChange(e as unknown as React.FocusEvent<HTMLInputElement>);
    }
  };
